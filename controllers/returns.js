...
const find_logs = async(req, res, next) => {
req.jobid = req.params.jobid
req.params.jobid = req.jobid
let jid = req.params.jobid
req.clientid = req.params.clientid
...
if (pageIt === 'true') {
    ...
    if (jid == null) {
        next(
            ApiError.badRequest(
                'jobid must be specified in the URL'
            )
        )
    } else {
        const results = {}
        ...
        if (endIndex < model.length) {
            let nextPage = page + 1
            results.next = `${baseUrl.url}/clients/${cid}/jobs/${jid}/returns/logs?paginate=true&page=${nextPage}&limit=${limit}`
        }
        if (startIndex > 0) {
            let prevPage = page - 1
            results.previous = `${baseUrl.url}/clients/${cid}/jobs/${jid}/returns/logs?paginate=true&page=${prevPage}&limit=${limit}`
        }
        try {
            let pool = await sql.connect(configJobData)
            results.data = await pool
                ...
                .input('limit', sql.Int, limit)
                .input('jid', sql.Int, jid)
                .execute('GetPaginatedReturnLogs')
            res.paginatedResults = results
            res.status(200).json({
                Next: res.paginatedResults.next,
                Previous: res.paginatedResults.previous,
                ReturnLogs: res.paginatedResults.data.recordset,
            })
            //res.paginatedResults
        } catch (err) {
            console.log({ Error: err.message })
    ...
    try {
        const jobid = req.params.jobid
        let pool = await sql.connect(configJobData)
        let getReturnLogs = await pool
            .request()
            .input('jid', sql.Int, jobid)
            .execute('GetReturnLogs')

        res.status(200).json(
            JSON.parse(
                getReturnLogs.recordset[0][
